...
class Board:
    ...
    def read_board(self, file, date):
        ...
        try:
            matter_list = list()
            # Regular expressions to capture the data
            court_pattern = r"(.*?)I\s*N\s*TH\s*E\s*CO\s*U\s*RT\s*OF.*|(.*?)BEFORE\s*THE\s*.*|(.*?)\s*THE\s*CO\s*U\s*RT\s*OF\s*.*"
            # court_pattern = r"(.*?)I\s*N\s*THE\s*CO\s*U\s*RT\s*OF\s*(.*?)\s*.*(\d\s*\d/\d\d/\d\d\d\d)*\s+C\.R\.\s*No:*\s*(\d+)|(.*?)BEFORE\s*THE\s*(.*?)\s*.*(\d\s*\d/\d\d/\d\d\d\d)*\s+C\.R\.\s*No:*\s*(\d+)|(.*?)\s*THE\s*CO\s*U\s*RT\s*OF\s*(.*?)\s*.*(\d\s*\d/\d\d/\d\d\d\d)*\s+C\.R\.\s*No:*\s*(\d+)"
            # case_stage_pattern = r"\s*\*\s*(.*?)\s*\*\s*"
