...
class MatterMatch:
    ...
    match_field: str   # 'petitioner_lawyer', 'respondent_lawyer', 'order_agp_names', etc.
    matched_text: str
...
class UserMatterMatcher:
    ...
    def __init__(self):
        ...
        self.name_prefixes = ['Mr.', 'Ms.', 'Mrs.', 'Dr.', 'Adv.', 'Smt.', 'Shri', 'Kumari']
    ...
    def find_user_matches_in_text(self, text: str, user_role: UserRole, field_context: str = '') -> List[Tuple[str, float]]:
        ...
        elif detected_role and self._is_compatible_role(detected_role, user_role.role_type):
            role_context_bonus = 0.05
        
        # Additional context bonus based on field type
        field_bonus = 0.0
        if field_context:
            field_bonus = self._get_field_context_bonus(field_context, user_role.role_type)
    ...
    def _get_field_context_bonus(self, field_context: str, user_role: str) -> float:
        ...
        if user_role in govt_roles:
            if 'respondent' in field_context.lower() or 'order_agp' in field_context.lower():
    ...
    def get_matters_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'medium_confidence_matches': len([m for m in matches if 0.75 <= m.confidence_score < 0.9]),
            'low_confidence_matches': len([m for m in matches if m.confidence_score < 0.75]),
            'match_sources': {
                'board_data': len([m for m in matches if m.match_source == 'board_data']),
                'order_analysis': len([m for m in matches if m.match_source == 'order_analysis'])
            },
            'recent_matches': len([m for m in matches if m.board_date and m.board_date >= '2024-01-01'])
        }
