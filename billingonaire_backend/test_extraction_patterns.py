...
def test_petitioner_patterns():
    ...
    patterns = [
        ("Pattern 1", r"([A-Z][a-zA-Z\s\.\-/]+?(?:\s+[Aa]lias\s+[A-Z][a-zA-Z\s/]+?)?)(?:\s*…\s*Petitioners?\s+)([A-Z][a-zA-Z\s\.\-/]+?)\s*(?:Versus|vs\.?)"),
        ("Pattern 2", r"IN\s+THE\s+MATTER\s+BETWEEN\s+([A-Z][a-zA-Z\s\.\-/]+?(?:\s+[Aa]lias\s+[A-Z][a-zA-Z\s\.\-/]+?)?)\s*…\s*Petitioners?"),
        ("Pattern 3", r"([A-Z][a-zA-Z\s\.\-/]+?(?:\s+[Aa]lias\s+[A-Z][a-zA-Z\s\.\-/]+?)?)\s*…\s*Petitioners?\s+(?:Versus|vs\.?)"),
        ("Pattern 4", r"([A-Z][a-zA-Z]+(?:\s+[a-zA-Z\.\-/]+){2,}(?:\s+[Aa]lias\s+[A-Z][a-zA-Z\s\.\-/]+)?)\s+(?:Versus|vs\.?)\s"),
        ("Pattern 5", r"([A-Z][a-zA-Z]+(?:\s+[a-zA-Z\.\-/]+){1,})\s+\.{2,}\s*PETITIONER"),
    ]
    ...
    for test in test_cases:
        ...
        for pattern_name, pattern in patterns:
            ...
            if match:
                ...
                extracted = re.sub(r"\s*\.{2,}\s*PETITIONER.*$", "", extracted, flags=re.IGNORECASE).strip()
                
                status = "✅ PASS" if extracted == test["expected"] else f"❌ FAIL (got: {extracted})"
                print(f"\n{test['name']}")
...
def test_respondent_patterns():
    ...
    # Pattern definitions from order_analyzer.py
    patterns = [
        ("Pattern 1", r"versus\s+((?:(?:Mr\.?|Ms\.?|Dr\.?|Shri?\.?|Smt\.?|The)\s+)?[A-Za-z\s\.\-&,]+?(?:\s+(?:And|&)\s+(?:Ors?\.?|Anr\.?))*?)\s*…\s*Respondents?"),
        ("Pattern 2", r"versus\s+((?:(?:Mr\.?|Ms\.?|Dr\.?|Shri?\.?|Smt\.?|The)\s+)?[A-Za-z\s\.\-&,]+?(?:\s+(?:And|&)\s+(?:Ors?\.?|Anr\.?))*?)(?:\s+(?:Mr\.|Ms\.|Adv\.|CORAM)|\n|$)"),
        ("Pattern 3", r"V/S\s+((?:The\s+)?[A-Za-z\s\.\-&,]+)\s+\.{2,}\s*RESPONDENT(?:\s+And\s+Ors)?"),
    ]
...
def test_agp_patterns():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "expected": ["Mr. O. A. Chandurkar, Addl. G.P.", "Mrs. G. R. Raghuwanshi, AGP"]
        }
    ]
    ...
    for test in test_cases:
        ...
        for match in matches:
            ...
            if re.search(r'AGP|GP|G\.?\s*P\.?', role1, re.IGNORECASE):
                # Normalize role
                role1_normalized = re.sub(r'Addl\.\s*Govt\.\s*Pleader', 'Addl. G.P.', role1, flags=re.IGNORECASE)
                role1_normalized = re.sub(r'Govt\.\s*Pleader', 'G.P.', role1_normalized, flags=re.IGNORECASE)
            ...
            if match[2]:  # a/w or with connector exists
                ...
                if re.search(r'AGP|GP|G\.?\s*P\.?', role2, re.IGNORECASE):
                    role2_normalized = re.sub(r'Addl\.\s*Govt\.\s*Pleader', 'Addl. G.P.', role2, flags=re.IGNORECASE)
                    role2_normalized = re.sub(r'Govt\.\s*Pleader', 'G.P.', role2_normalized, flags=re.IGNORECASE)
                    role2_normalized = re.sub(r'\s+', ' ', role2_normalized).strip()
                    extracted_agps.append(f"{name2}, {role2_normalized}")
        
        status = "✅ PASS" if extracted_agps == test["expected"] else f"❌ FAIL"
        print(f"\n{test['name']}")
        print(f"  Expected: {test['expected']}")
...
def test_categorization():
    ...
    for test in test_cases:
        ...
        is_non_hearing = any(re.search(p, test["text"], re.IGNORECASE) for p in no_hearing_patterns)
        
        # Check for disposal
        is_disposed = re.search(r"disposed\s+of", test["text"], re.IGNORECASE) is not None
