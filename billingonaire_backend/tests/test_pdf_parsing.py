    ...
    try:
        ...
        with open(pdf_path, 'rb') as file:
            ...
            try:
                ...
                print(f"⚠️  Board initialization failed (expected in test environment): {e}")
            ...
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    for i, record in df.head(5).iterrows():
                        print(f"Record {i+1}:")
                        print(f"  Case: {record.get('case_type', 'N/A')}/{record.get('case_no', 'N/A')}/{record.get('case_year', 'N/A')}")
                        print(f"  Serial: {record.get('serial_number', 'N/A')}")
                        print(f"  Petitioner: {record.get('petitioner_lawyer', 'N/A')[:50]}...")
                        print(f"  Respondent: {record.get('respondent_lawyer', 'N/A')[:50]}...")
                        print()
                    ...
                    if 'serial_number' in df.columns:
                        empty_serials = df['serial_number'].isna().sum() + (df['serial_number'] == '').sum()
                        print(f"  Empty serial numbers: {empty_serials} out of {record_count}")
