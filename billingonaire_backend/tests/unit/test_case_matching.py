...
from order_analyzer import OrderDocumentAnalyzer, CaseInfo, OrderAnalysisResult
...
class TestCaseMatching:
    ...
    def test_case_info_structure(self):
        ...
        assert isinstance(case.government_pleader, list), "government_pleader must be list"
    ...
    def test_multi_case_extraction(self):
        ...
        assert len(result.order_cases) == 3, f"Expected 3 cases, got {len(result.order_cases)}"
    ...
    def test_parse_case_reference_types(self):
        ...
        for case_ref, expected in test_cases:
            ...
            # Verify types
            assert isinstance(case_type, str), f"case_type must be str, got {type(case_type)}"
