...
def test_case_extraction():
    ...
    matches = re.findall(case_block_pattern, sample_order_text, re.DOTALL | re.IGNORECASE)
    ...
    for idx, (case_num, year, block_text) in enumerate(matches):
        ...
        resp_match = re.search(respondent_pattern, block_text, re.DOTALL | re.IGNORECASE)
        ...
        print(f"\n  Validation:")
        print(f"    Petitioner match: {petitioner == expected['petitioner'] if 'petitioner' in locals() else 'SKIPPED'}")
        print(f"    Respondent match: {respondent == expected['respondent'] if 'respondent' in locals() else 'SKIPPED'}")
...
def test_order_category():
    ...
    if re.search(r"(?:Adv\.|Ms\.|Mr\.)\s+[A-Za-z\s\.]+(?:AGP|GP)", sample_order_text, re.IGNORECASE):
