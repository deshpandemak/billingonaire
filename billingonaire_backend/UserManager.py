...
class UserManager:
    def __init__(self):
        ...
        self.role_display_names = {"admin": "Administrator", "user": "User"}
    ...
    def setup_initial_admin(self) -> Dict:
        ...
        try:
            ...
            if existing_users:
                ...
                user_ref = self.db.collection(self.users_collection).document(
                    user_doc.id
                )
                user_ref.update({"role": "admin", "updated_at": datetime.now()})
                ...
                return {
                    "message": f"User {admin_email} needs to log in first to be made admin"
                }

        except Exception as e:
            logging.error(f"Error setting up initial admin: {str(e)}")
            raise HTTPException(
                status_code=500, detail=f"Error setting up admin: {str(e)}"
            )
    ...
    def create_user_profile(
        ...
        email: str,
        role: str = "user",
        legal_category: str = "assistant_government_pleader",
        full_name: str = None,
    ) -> Dict:
        """
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error creating user profile: {str(e)}"
            )
    ...
    def get_user_profile(self, uid: str) -> Dict:
        ...
        try:
            ...
            if "created_at" in user_data and hasattr(
                ...
                user_data["created_at"] = user_data["created_at"].strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            if "updated_at" in user_data and hasattr(
                user_data["updated_at"], "strftime"
            ):
                user_data["updated_at"] = user_data["updated_at"].strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
    ...
    def admin_update_user_profile(
        self, target_uid: str, updates: Dict, admin_uid: str
    ) -> Dict:
        ...
        try:
            ...
            if "role" in updates:
                ...
                if role not in self.valid_roles:
                    raise HTTPException(
                        status_code=400,
                        detail=f"Invalid role: {role}. Must be one of: {', '.join(self.valid_roles)}",
                    )
        ...
        try:
            ...
            if role == "admin":
                ...
                if not full_name:
                    raise HTTPException(
                        status_code=403,
                        detail="Full name required. Please update your profile.",
                    )
                return full_name
    ...
    def get_active_user_names(self) -> List[str]:
        ...
        try:
            ...
            logging.info(
                f"📋 get_active_user_names: Found {len(all_users)} total users in collection"
            )
            ...
            logging.info(
                f"📤 get_active_user_names returning {len(active_user_names)} users: {sorted(active_user_names)}"
            )
    ...
    def match_user_name_to_agp(
        self, user_name: str, agp_names_in_data: List[str]
    ) -> tuple:
        """
        ...
        logging.info(
            f"🔍 Fuzzy matching '{user_name}' against {len(agp_names_in_data)} AGP names"
        )
        ...
        for agp_name in agp_names_in_data:
            ...
            if len(user_words) > 0 and len(agp_words) > 0:
                ...
                if last_name_score < 0.60:  # Slightly lenient for spelling variations
                    logging.debug(
                        f"   ⏭️  Skipping '{agp_name}' - last name '{agp_last}' didn't match (score: {last_name_score:.2f})"
                    )
                    continue

                logging.debug(
                    f"   ✅ '{agp_name}' - last name match: {last_name_score:.2f}"
                )
        ...
        if best_match:
            logging.info(
                f"✅ Best match found: '{best_match}' with score {best_score:.2%}"
            )
        else:
            logging.warning(
                f"⚠️ No match found for '{user_name}' among {len(agp_names_in_data)} AGP names"
            )
    ...
    def match_user_name_to_all_agps(
        ...
        """
        Find all AGP names that match *user_name* above *threshold*.
        Returns list of (agp_name, confidence_score) tuples sorted by
        confidence (highest first).

        Delegates to :func:`UserMatterMatcher.score_name_match` so the scoring
        logic is defined in one place and shared with Board.getData.
        """
        try:
        ...
        all_matches = [
            (name, score_name_match(user_name, name))
            for name in agp_names_in_data
            if name
        ]
        all_matches = [
            (name, score) for name, score in all_matches if score >= threshold
        ]
    ...
    def list_firebase_auth_users(self) -> List[Dict]:
        ...
        try:
            ...
            while page:
                for user in page.users:
                    ...
                    if user.user_metadata.last_sign_in_timestamp:
                        last_signin_timestamp = datetime.fromtimestamp(
            ...
            raise HTTPException(
                status_code=500, detail=f"Error listing Firebase Auth users: {str(e)}"
            )
    ...
    def sync_firebase_users_to_firestore(self, admin_uid: str) -> Dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error syncing users: {str(e)}"
            )
    ...
    def get_firebase_auth_users_not_in_firestore(self) -> List[Dict]:
        ...
        try:
            ...
            for firebase_user in firebase_users:
                if (
                    firebase_user["uid"] not in existing_firestore_users
                    and firebase_user["email"]
                ):
            ...
            raise HTTPException(
                status_code=500, detail=f"Error getting unsynced users: {str(e)}"
            )
