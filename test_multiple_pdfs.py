...
def main():
    ...
    print("\n📊 Results Summary:")
    print("-" * 80)
    print(f"{'PDF Name':<40} {'Records':<8} {'Old Logic':<10} {'Empty S#':<8} {'Lost?':<6}")
    print("-" * 80)
    
    for result in results:
        if 'error' not in result:
            lost_indicator = "⚠️ YES" if result['would_lose_records'] else "✅ NO"
            print(f"{result['name'][:39]:<40} {result['total_records']:<8} {result['old_logic_records']:<10} {result['empty_serials']:<8} {lost_indicator:<6}")
        else:
            print(f"{result['name'][:39]:<40} {'ERROR':<8} {result.get('error', '')[:20]:<10}")
    ...
    if problem_cases:
        ...
        for case in problem_cases:
            print(f"  - {case['name']}: {case['total_records']} → {case['old_logic_records']} records")
    else:
