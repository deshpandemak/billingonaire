...
- **Technical Implementations**:
    ...
    - **Bill Generation with Role-Based Access Control & AI/ML Name Matching**: Comprehensive bill generation workflow where non-admin users can only generate bills for their own assigned cases, while admins can generate bills for any AGP. The system features an intelligent AI/ML-powered fuzzy name matching system with 75% confidence threshold that maps user names from the system to AGP names in board data. The matching algorithm requires both first name (exact or initial) AND last name (exact) matches to prevent false positives (e.g., prevents "Rajesh Patil" from matching "Sneha Patil"). Scoring weights: first name 40%, last name 30%, sequence similarity 20%, word overlap 10%. When confidence is below 75%, returns detailed error with best match candidate to prevent misbilling. AGP dropdown shows active system users (from users collection) rather than board data names. Fee calculation based on order analysis (ADJOURNED: ₹1,250, HEARD & ADJN.: ₹1,875, DISPOSED: ₹2,500) using correct database field names (order_category, order_text, order_disposal_reason). Updated: October 3, 2025.
